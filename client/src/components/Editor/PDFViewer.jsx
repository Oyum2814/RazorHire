...
const PDFViewer = ({
  ...
  const sourceCanvasRef = useRef(null); // Keep track of the INTERNAL canvas used by the renderer. We use this as a buffer. The actual preview on the webpage is done by another canvas.
  ...
  useEffect(() => {
    ...
    const targetCanvas = document.getElementById("targetCanvas"); // The actual canvas displayed on the webpage, once the buffer canvas is rendered, we copy the contents here. This avoids jitter caused by PDF refresh
    ...
    >
      <div className="pdf-layout w-full h-full  flex flex-col justify-center  items-center">
        ...
        >
          <Doc
            ...
            loading={() => {
              ...
              return (
                <svg
                  aria-hidden="true"
                  className="shadow-none border-none w-16 h-16 text-gray-200 animate-spin  fill-white"
                  viewBox="0 0 100 101"
                  fill="none"
          ...
          >
            <Pag
              ...
              onRenderSuccess={() => {
                setLoading(false);
              }} // Once rendered we can change loading to false and update buffer
              pageNumber={1}
            />
          </Doc>
        </div>
        {!loading && (
          ...
          >
            {" "}
            {/*Directly use the blob url to download PDF*/}
            Save as PDF
          </a>
        )}
      </div>
